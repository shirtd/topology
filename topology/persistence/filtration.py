...
class Filtration:
    def __init__(self, K, key, reverse):
        ...
        self.imap = {s.astuple() : i for i, s in enumerate(self)}
    def __len__(self):
    ...
    def sort_faces(self, K, i):
        return sorted([self.index(f) for f in K.get_faces(self[i])])
    def sort_cofaces(self, K, i):
        return sorted([self.index(f) for f in K.get_cofaces(self[i])], reverse=True)
    ...
    def get_chains(self, K, rng):
        return {i : self.as_chain(K, i) for i in rng}# for i in I}
    def get_cochains(self, K, rng):
        return {i : self.as_cochain(K, i) for i in rng}# for i in I}
